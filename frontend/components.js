...
function createEditPanel(internship) {
    ...
    saveButton.onclick = async () => {
        ...
        if (await updateInternship(internship.id, {role: newRole, deadline: newDeadline, location: newLocation, jdlink: newLink})) {
            renderList();
        }
    }
    return container;
...
function createDeleteButton(internship) {
        ...
        deleteButton.onclick = async () => { 
            ...
            if (await deleteInternship(internship.id)) { renderList(); }
        }
